...
# Dirección de correo del destinatario (en este caso, es el mismo que el remitente)
...
def redimensionar_imagen(ruta, width, height):
    try:
        ...
        messagebox.showerror("@javier - Error", f"No se pudo cargar la imagen: {str(e)}")
        return None
...
def menu_principal():
    ...
    # Añadir el icono de la ventana usando una ruta absoluta
    icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'icono.ico')
    if os.path.exists(icon_path):
        root.iconbitmap(icon_path)
    else:
        messagebox.showwarning("@javier - Advertencia", "El archivo de icono no se encontró.")
    ...
    if os.path.exists(ruta_logo):
        ...
        # Mostrar la imagen redimensionada si se cargó correctamente
        ...
        messagebox.showwarning("@javier - Advertencia", "El archivo de imagen no se encontró.")
